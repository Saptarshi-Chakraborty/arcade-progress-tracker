...
const ParticipantsBody = () => {
  ...
  // Fetch report dates when facilitator data is available
  useEffect(() => {
    if (facilitator) {
      fetchReportDates();
    }
  ...
  // Fetch participants when a date is selected
  useEffect(() => {
    if (selectedDate) {
      fetchParticipantsForDate(selectedDate);
    }
  }, [selectedDate]);
  ...
  const fetchReportDates = async () => {
    ...
    try {
      ...
      if (response.documents.length > 0) {
        setReportDates(response.documents.map(doc => ({
          id: doc.$id,
          date: doc.reportDate,
        ...
        setIsLoading(false);
        toast.error('No reports found for your facilitator code.');
      }
    } catch (error) {
      console.error('Error fetching report dates:', error);
      toast.error('Failed to fetch report dates');
      setIsLoading(false);
    }
  };

  const fetchParticipantsForDate = async (dailyReportId) => {
    setIsLoading(true);
    try {
      const response = await appwrite.database.listDocuments(
        appwrite.DATABASE.ID,
        ...
        [
          appwrite.Query.equal('dailyReport', dailyReportId),
          appwrite.Query.limit(500) // Adjust as needed
        ]
      );

      setParticipants(response.documents);
      setFilteredParticipants(response.documents);
      setIsLoading(false);
    } catch (error) {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md mb-6">
        <div className="p-6 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            {/* Refresh Button */}
            <button
              onClick={() => selectedDate && fetchParticipantsForDate(selectedDate)}
...
export default ParticipantsBody;